...
from app.controllers.producto_controllers import ProductoController
...
def get_producto(id_producto):
    producto = ProductoController.get_producto(id_producto)
    if producto:
...
def update_producto(id_producto):
    data = request.get_json()
    producto = ProductoController.update_producto(id_producto, data)
    if producto:
        return jsonify(producto.to_dict()), 200
...
def delete_producto(id_producto):
    success = ProductoController.delete_producto(id_producto)
    if success:
